...
ggplot(data, aes(x = Group, y = Infection_Rate, fill = Group)) +
  ...
  labs(title = "Figure 1: COVID-19 Infection Rates\nin Pfizer-BioNTech Study Groups",
       y = "Infection Rate (%)",
       x = "Group") +
...
We employ a likelihood‐based inference method, as recommended by Barnard et al.(196). For likelihood inference, we need to check CI using large number and bootstrap, for p-value, use chi square distribution and empirical p-value.
...
Under $H_0$, the larger the ratio, the stronger is the evidence against $H_0$. The second statistic, Log likelihood ratio, follows a chi-square distribution with one degree of freedom under $H_0$. $W \sim \chi^2_1 = 2 \ln(\Lambda)$. 

After calculating $W$, we compute the p-value as $P(\chi^2_1 \ge W)$, to observe if p value is less than 0.05. 
...
hist(
  VE_boot, breaks = 30,
  main = "Figure 2: Bootstrap Distribution of Vaccine Efficacy",
...
plot(ml.binom) +
  labs(
    title    = "Figure 3: Second order approximation\n to Binomial Likelihood",
    subtitle = "n = 170, x = 8",
    x        = expression(psi)
  )
```
...
hist(Wstar, breaks=30, main="Figure 4: Histogram of W* under H0", xlab="W*")
...
ggplot() +
  ...
  labs(
    title    = "Figure 5: Prior and Posterior Distribution of Vaccine Efficacy (Pi)",
    x        = expression(pi),
    y        = "PDF"
  ) +
  theme_minimal()
...
ggplot(psi_df, aes(x = psi_samples)) +
  ...
  labs(
    title    = "Figure 6: Posterior Distribution of Vaccine Efficacy (Psi)",
    x        = expression(psi),
    y        = "Density"
  ) +
  theme_minimal()
...
# Bibliography
1. Barnard GA, Jenkins GM, Winsten CB. Likelihood inference and time series. J R Stat Soc Ser A Stat Soc. 1962;125(3):321-52. 

2. Gelman A, Carlin JB, Stern HS, Dunson DB, Vehtari A, Rubin DB. Bayesian Data Analysis. 3rd ed. Chapman and Hall/CRC; 2013. 

3. Venu AX. Highest Posterior Density Intervals As Analogues to Profile Likelihood Ratio Confidence Intervals for Modes of Unimodal Distributions. arXiv Prepr arXiv:2412.06528. 2024.
...
### Code to visualize Prior and Posterior Distribution of Vaccine Efficacy (Pi)
```{r ref.label = "PiDis", eval = FALSE}
```

### Code to visualize Posterior Distribution of Vaccine Efficacy (Psi)
```{r ref.label = "PsiDIS", eval = FALSE}
```

### Code to calculate the median of posterior (Psi)
```{r ref.label = "median", eval = FALSE}
```

### Code to find quantile-based credible interval of posterior (Psi)
```{r ref.label = "quan_CI", eval = FALSE}
```

### Code to find HPDI credible interval of posterior (Psi)
```{r ref.label = "HDPI_CI", eval = FALSE, warning = FALSE}
```

### Code to find p-value of posterior (Psi)
```{r ref.label = "prob_0.3", eval = FALSE}
```
